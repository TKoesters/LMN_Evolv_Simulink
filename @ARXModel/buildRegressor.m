...
%   Detailed explanation goes here
    ...
    regressorIn = ones(N+1,obj.inputDelays).*input(1); % fill with first input
    regressorOut = ones(N+1,obj.outputDelays) .*output(1); % fill with first output better for ranges
    for row = 1 : N
        for collIn = 1 : obj.inputDelays
            if 1+row-collIn>0
        ...
        for collOut = 1:obj.outputDelays
            if 1+row-collOut>0
